        ...
        using (new EditorGUILayout.VerticalScope("box"))
        {
            ...
            if (UIComponnets != null)
            {
                foreach (var item in UIComponnets)
                {
                    using (new EditorGUILayout.HorizontalScope("box"))
                    {
                        var Gami_Sound = item.gameObject.GetComponent<GAMISound>();
                        bool hasSoundCompoent = Gami_Sound != null;
                        ...
                        if (!hasSoundCompoent)
                        {
                            if (GUILayout.Button("Add " + typeof(GAMISound).Name)) { item.gameObject.AddComponent<GAMISound>(); }
                        }
                        else
                        {
                            if (GUILayout.Button("Remove " + typeof(GAMISound).Name)) { DestroyImmediate(Gami_Sound); }
                        }
